...
usage()
{
    echo -e "\nUsage: source fvc-setup-release.sh
...
#FVC Layer
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-fvc\"" >> conf/bblayers.conf

#agility_carrier_board_support
if [ -n "$AGILITY_DEMO" ]; then
echo "BBLAYERS += \"\${BSPDIR}/sources/meta-agility-demo\"" >> conf/bblayers.conf
fi
